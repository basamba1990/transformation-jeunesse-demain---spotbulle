...
const PodsPage: React.FC = () => {
    ...
    const loadPods = useCallback(async () => {
        ...
        try {
            ...
            logError("Error loading pods:", err);
            setError(err.message || "Impossible de charger les pods. Veuillez réessayer.");
        } finally {
            setIsLoading(false);
        }
    }, [viewMode, user]);
    ...
    const handleDeletePod = async (podId: number) => {
        ...
        try {
            ...
            setSuccessMessage("Pod supprimé avec succès !");
            ...
            logError("Error deleting pod:", err);
            setError(err.message || "Erreur lors de la suppression du pod.");
        }
    };
    ...
    const handleTranscribePod = async (podId: number) => {
        ...
        try {
            ...
            // Afficher un message de succès
            setSuccessMessage("Transcription terminée et enregistrée !");
            ...
            logError(`Error transcribing pod ${podId}:`, err);
            setError(err.message || "Erreur lors de la transcription du pod.");
        } finally {
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            ...
            <div className="mb-6 flex justify-center space-x-4">
                ...
                >
                    Tous les Pods {pods.length > 0 ? `(${pods.length})` : ""}
                </button>
                {user && (
                    <button
                    ...
                    >
                        Mes Pods {myPods.length > 0 ? `(${myPods.length})` : ""}
                    </button>
                )}
            </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-neutral-800 dark:text-neutral-200">
                                            {selectedPod.created_at && formatDate(selectedPod.created_at)}
                                        </span>
                                    </div>
                                    <div>
                                        <span className="text-neutral-500 dark:text-neutral-400">Mis à jour le: </span>
                                        <span className="text-neutral-800 dark:text-neutral-200">
                                            {selectedPod.updated_at && formatDate(selectedPod.updated_at)}
                                        </span>
                                    </div>
                                    <div>
                                        <span className="text-neutral-500 dark:text-neutral-400">Propriétaire: </span>
                                        <span className="text-neutral-800 dark:text-neutral-200">ID {selectedPod.owner_id}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                                {!selectedPod.transcription && (
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-center py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {isTranscribing[selectedPod.id] ? (
                                            <>
                                            ...
                                            <>
                                                <FileText size={16} className="mr-2" /> Transcrire
                                            </>
                                        )}
                                    </button>
                                )}
                                
                                {user && selectedPod.owner_id === user.id && (
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            handleDeletePod(selectedPod.id);
                                        }}
                                        className="flex-1 flex items-center justify-center py-2 px-4 bg-red-500 hover:bg-red-600 text-white rounded-md"
                                    >
            ...
            {/* Bouton flottant pour créer un pod */}

...
import type { DISCResults } from "../schemas/disc_schema"; // << utilise DISCResults uniquement
...
interface ProfileData {
  ...
  disc_assessment_results: DISCResults | null; // << corrigé ici
}
...
const ProfilePage: React.FC = () => {
  ...
  const loadProfileData = async () => {
    try {
      ...
      if (profileData.disc_type) {
        const results = await discService.getResults(); // << corrigé ici
  ...
  return (
    <div className="container mx-auto p-4 bg-neutral-lightest min-h-screen">
      {/* ... (contenu JSX conservé et fonctionnel) */}
    </div>
  );
};

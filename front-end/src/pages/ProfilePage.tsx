...
import { fetchMyProfile, updateMyProfile, updateCurrentUserProfile, getMyDiscResults, submitMyDiscAssessment, getDiscQuestionnaire } from "../services/api";
...
import { DISCOnboardingComponent } from "../profile/DISCOnboardingComponent"; // Assurez-vous que le chemin est correct
import { DISCResults, DISCScores } from "../schemas/disc_schema"; // Assurez-vous que le chemin est correct

interface ProfileData {
    user_id: number;
    bio: string | null;
    interests: string[] | null; // Changé en string[] pour mieux gérer les tags
    skills: string[] | null;    // Changé en string[]
    objectives: string | null;
    profile_picture_url: string | null;
    disc_type: string | null;
    disc_assessment_results: Record<string, number> | null; // Pourrait être DISCScores
    // Ajoutez d_autres champs de profil ici
}
...
// Input component (peut être déplacé dans son propre fichier ui/Input.tsx si réutilisé)
        ...
        <div>
            ...
            <div className="relative">
                ...
                <input
                    id={id}
                    type={type}
                    className={`block w-full pl-${icon ? '10' : '3'} pr-3 py-2 border border-neutral-light rounded-md shadow-sm placeholder-neutral-default focus:outline-none focus:ring-primary focus:border-primary sm:text-sm ${props.readOnly ? 'bg-neutral-lightest cursor-not-allowed' : ''} ${className}`}
                    {...props}
                />
            </div>
        </div>
...
const ProfilePage: React.FC = () => {
        ...
        try {
            ...
            if (editableUser.email !== user?.email || editableUser.full_name !== user?.full_name) {
                ...
                if (Object.keys(userUpdatePayload).length > 0 && user?.id) {
                  updatedUser = await updateCurrentUserProfile(user.id, userUpdatePayload); // Assurez-vous que cette API existe et prend user.id
                }
            }
            ...
            if (token && updatedUser) {
                login(token, updatedUser); // Mettre à jour le contexte Auth
            }
            ...
            if (err.response && err.response.data && err.response.data.detail) {
                errorMessage = Array.isArray(err.response.data.detail) ? err.response.data.detail.map((e: any) => e.msg).join(", ") : err.response.data.detail;
            }
    ...
    const handleDiscCompleted = async () => {
        ...
        try {
            ...
            // Recharger le profil pour obtenir le disc_type mis à jour
            const profileData = await fetchMyProfile();
            setProfile(profileData);
        } catch (error) {
            console.error("Erreur lors de la récupération des résultats DISC après complétion:", error);
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 lg:p-8 bg-neutral-lightest min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Colonne Informations Personnelles et Profil */} 
                ...
                <div className="md:col-span-1 space-y-6">
                    ...
                    {/* D'autres cartes pourraient être ajoutées ici: Stats, Badges, etc. */}
                </div>
            </div>

...
def get_embedding_sbert(text: str) -> Optional[List[float]]:  
    """Génère un embedding avec SentenceTransformer."""  
...
def get_pod_embedding(db: Session, pod_id: int, use_openai: bool = False) -> Optional[List[float]]:  
    """Récupère ou génère et sauvegarde l'embedding du pod."""  
    ...
    if embedding:  
        pod.embedding = embedding  # Sauvegarde  
        db.commit()  
    return embedding  
...
async def find_ia_matches(  
    ...
    current_profile = current_user.profile[0]  # Correction ici  
    ...
    for other_user in all_users:  
        ...
        total_score = (0.25 * disc_score + 0.25 * interests_score + 0.25 * objectives_score + 0.25 * content_score)  
